...
VUE,
      ...
      <el-form-item label="{$colComment}" prop="{$name}">
        <el-input v-model="form.{$name}" placeholder="请输入图片地址">
          <template #append>
...
PHP;
    ...
    /**
     * 以插件模式生成代码
     * 所有文件输出到 plugins/{moduleName}/ 下，附带安装脚本和 README
     */
    public function generateAsPlugin(array $config): array
    {
        $moduleName  = $config['module_name'];
        $modelName   = $config['model_name'];
        $tableComment = $config['table_comment'] ?? $modelName;
        ...
        if (empty($columns)) {
            $columns = $this->getTableColumns($config['table_name']);
        }

        // 先用标准 generate 获取所有文件内容
        $standardFiles = $this->generate($config);
        $pluginDir = "plugins/{$moduleName}";
        $kebab = $this->toKebab($modelName);
        ...
        // 将标准文件重新映射到插件目录
        $pathMapping = [
            'model'      => "server/app/model/{$moduleName}/{$modelName}.php",
            'repository' => "server/app/repository/{$moduleName}/{$modelName}Repository.php",
            'service'    => "server/app/service/{$moduleName}/{$modelName}Service.php",
            'controller' => "server/app/adminapi/controller/v1/{$moduleName}/{$modelName}Controller.php",
            'validate'   => "server/app/adminapi/validate/v1/{$moduleName}/{$modelName}Validate.php",
            'route'      => "server/app/adminapi/route/{$moduleName}.php",
            'api'        => "admin/src/api/{$kebab}.ts",
            'page'       => "admin/src/views/{$moduleName}/{$kebab}/index.vue",
            'form'       => "admin/src/views/{$moduleName}/{$kebab}/components/{$modelName}Form.vue",
            'migration'  => $standardFiles['migration']['path'] ?? "server/database/migrations/create_{$config['table_name']}_table.php",
        ];

        foreach ($standardFiles as $key => $file) {
            $relativePath = $pathMapping[$key] ?? $file['path'];
            $files[$key] = [
        ...
        // 生成安装脚本
        $files['install'] = [
            'path'    => "{$pluginDir}/install.php",
            'content' => $this->generatePluginInstallScript($moduleName, $modelName, $pathMapping),
        ];

        // 生成 plugin.json 元数据
        $files['meta'] = [
            'path'    => "{$pluginDir}/plugin.json",
            'content' => $this->generatePluginMeta($moduleName, $modelName, $tableComment),
        ];

        // 生成 README
        $files['readme'] = [
            'path'    => "{$pluginDir}/README.md",
            'content' => $this->generatePluginReadme($moduleName, $modelName, $tableComment),
        ];
     ...
     * 生成插件安装脚本
     */
    protected function generatePluginInstallScript(string $module, string $model, array $pathMapping): string
    {
        $copies = '';
        foreach ($pathMapping as $key => $relativePath) {
            $copies .= "    '{$key}' => '{$relativePath}',\n";
        }
 ...
 * {$model} 插件安装脚本
 *
 * 用法: php think plugin:install {$module}
 * 或手动执行: php plugins/{$module}/install.php
 */
declare(strict_types=1);

\$pluginDir = __DIR__;
\$rootDir   = dirname(\$pluginDir, 2); // 项目根目录（与 server/ 同级）
\$serverDir = \$rootDir . '/server';

// 文件映射：插件内相对路径 => 项目内目标路径
\$fileMap = [
{$copies}];
...
foreach (\$fileMap as \$key => \$relativePath) {
    \$source = \$pluginDir . '/' . \$relativePath;
    ...
    if (str_starts_with(\$relativePath, 'admin/')) {
        \$target = \$rootDir . '/' . \$relativePath;
    } elseif (str_starts_with(\$relativePath, 'server/')) {
        \$target = \$rootDir . '/' . \$relativePath;
    } else {
        \$target = \$serverDir . '/' . \$relativePath;
    }

    if (file_exists(\$target)) {
        echo "  [跳过] {\$relativePath} (已存在)\\n";
        \$skipped++;
        continue;
    }
    ...
    copy(\$source, \$target);
    echo "  [安装] {\$relativePath}\\n";
    \$installed++;
}

echo "\\n安装完成: 安装 {\$installed} 个文件, 跳过 {\$skipped} 个文件\\n";
echo "请执行以下操作:\\n";
echo "  1. php think migrate:run  (执行数据库迁移)\\n";
echo "  2. 在后台菜单中添加对应的菜单项\\n";
echo "  3. 配置对应的权限标识\\n";
PHP;
    }
     ...
     * 生成插件元数据
     */
    protected function generatePluginMeta(string $module, string $model, string $comment): string
    {
        return json_encode([
            'name'        => $module,
            'title'       => $comment . '管理',
            'description' => "提供{$comment}的完整 CRUD 功能，包含后端接口和前端管理页面",
            'version'     => '1.0.0',
            'author'      => '',
            'require'     => [
                'php'       => '>=8.0',
     ...
     * 生成插件 README
     */
    protected function generatePluginReadme(string $module, string $model, string $comment): string
    {
        return <<<MD
# {$comment}管理插件
...
```bash
# 方式一：使用安装脚本
php plugins/{$module}/install.php
...
| 类型 | 文件 |
|------|------|
| Model | `app/model/{$module}/{$model}.php` |
| Repository | `app/repository/{$module}/{$model}Repository.php` |
| Service | `app/service/{$module}/{$model}Service.php` |
| Controller | `app/adminapi/controller/v1/{$module}/{$model}Controller.php` |
| Validate | `app/adminapi/validate/v1/{$module}/{$model}Validate.php` |
| Route | `app/adminapi/route/{$module}.php` |
| Migration | `database/migrations/create_*_table.php` |
| Frontend API | `admin/src/api/*.ts` |
| Frontend Page | `admin/src/views/{$module}/*/index.vue` |
| Frontend Form | `admin/src/views/{$module}/*/components/{$model}Form.vue` |
MD;
    }
}
