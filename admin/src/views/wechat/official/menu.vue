            ...
            <div class="menu-editor">
                ...
                <div class="menu-list">
                    <div class="menu-tabs">
                        ...
                        >
                            {{ menu.name || $t('channel.menuDefault') + (idx + 1) }}
                    ...
                    >
                        ...
                        >
                            {{ sub.name || $t('channel.subMenuDefault') + (sIdx + 1) }}
                            <el-icon
                                class="del-icon"
                        ...
                        >
                            <el-icon><Plus /></el-icon> {{ $t('channel.addSubMenu') }}
                        </div>
                    </div>
                </div>
                ...
                <div v-if="currentMenu" class="menu-form">
                    <el-form label-width="100px" label-position="left">
                        <el-form-item :label="$t('channel.menuName')">
                            <el-input v-model="currentMenu.name" maxlength="16" show-word-limit />
                        </el-form-item>
                        <el-form-item :label="$t('channel.menuType')">
                            <el-select v-model="currentMenu.type" style="width: 100%">
                                <el-option :label="$t('channel.jumpWebpage')" value="view" />
                                <el-option :label="$t('channel.clickEvent')" value="click" />
                                <el-option :label="$t('channel.jumpMiniProgram')" value="miniprogram" />
                            </el-select>
                        </el-form-item>
                        <el-form-item v-if="currentMenu.type === 'view'" :label="$t('channel.webpageUrl')">
                            <el-input v-model="currentMenu.url" placeholder="https://" />
                        </el-form-item>
                        <el-form-item v-if="currentMenu.type === 'click'" :label="$t('channel.eventKey')">
                            <el-input v-model="currentMenu.key" :placeholder="$t('channel.customEventId')" />
                        </el-form-item>
                        <template v-if="currentMenu.type === 'miniprogram'">
                            <el-form-item :label="$t('channel.miniAppId')">
                                <el-input v-model="currentMenu.appid" />
                            </el-form-item>
                            <el-form-item :label="$t('channel.miniPath')">
                                <el-input v-model="currentMenu.pagepath" />
                            </el-form-item>
                            <el-form-item :label="$t('channel.fallbackUrl')">
                                <el-input
                                    v-model="currentMenu.url"
                                    :placeholder="$t('channel.fallbackUrlPlaceholder')"
                                />
                            </el-form-item>
                        </template>
                    </el-form>
...
const handleSaveMenu = async () => {
    if (menuData.length === 0) {
        ElMessage.warning(t('channel.addAtLeastOneMenu'))
        return
    }
    try {
        saving.value = true
        await officialAccountApi.createMenu({ button: menuData })
        ElMessage.success(t('channel.menuPublishSuccess'))
    } catch (e: any) {
        ElMessage.error(e?.message || t('channel.publishFailed'))
    } finally {
        saving.value = false
    }
}
...
const handleDeleteMenu = async () => {
    try {
        await ElMessageBox.confirm(t('channel.deleteCustomMenuConfirm'), t('channel.tip'), { type: 'warning' })
        deleting.value = true
        await officialAccountApi.deleteMenu()
        menuData.splice(0, menuData.length)
        ElMessage.success(t('channel.menuDeleted'))
    } catch (e) {
